...
pipeline {
    ...
    stages {
        ...
        stage('Installing packages') {
            steps {
                script {
                    sh '/usr/bin/python -m pip install -r playbooks/files/requirements_test.txt'
                }
            }
        }
        ...
        stage('Static Code Checking') {
            steps {
                script {
                    sh 'find . -name \\*.py | xargs /usr/bin/python -m pylint --load-plugins=pylint_django -f parseable | tee pylint.log'
                    recordIssues(
            ...
            steps {
                withCredentials([gitUsernamePassword(credentialsId: 'gitea_repo', gitToolName: 'git')]) {
                    sh """
                        ...
                        cd app && /usr/bin/python -m bumpversion --config-file setup.cfg --allow-dirty --verbose minor --list > build_vars.env
